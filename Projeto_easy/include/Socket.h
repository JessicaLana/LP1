...
class Socket{
    ...
    public:
    virtual void sendData(const char* data_, int size) = 0;
};
class SocketCliente : public Socket{
    ...
    public:
    SocketCliente(std::string ip,unsigned long port); /*!< Initializes ip server address and port. Does NOT establish connection*/
    void connectToServer(); /*!< Establishes connection to server */
    void sendData(const char* data_, int size); /*!< Sends arbritrary data - use the sizeof operator*/
    SocketCliente(){}; /*!< Default constructor */
    ~SocketCliente(); /*!< Closes connection */
    int getFides();
};
#endif